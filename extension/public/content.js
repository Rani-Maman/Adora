...
(function () {
  ...
  let authSupported = true; // false on Safari (no chrome.identity); set from background at init
    ...
    return `
      * { margin: 0; padding: 0; box-sizing: border-box; }
      :host {
        ...
        /* Cap to the viewport so the panel is never cut off on phone screens */
      ...
      .adora-widget {
        ...
        max-height: 80dvh; /* iOS Safari: vh ignores the collapsing toolbars */
        border-radius: 14px;
        background: ${t.bg};
  ...
  function buildExpandedHTML() {
    ...
    // Skipped where auth is impossible (Safari): show the verdict UI logged-out.
    if (!authUser && authSupported) {
    ...
    if (!siteResult) {
      ...
      // Voting needs a user token, so omit it entirely where auth is impossible.
      if (authSupported) {
  ...
  function createWidget() {
    ...
    // Clamp to the viewport — a position saved on a wider screen (or a drag that
    // ran off the edge) would otherwise strand the widget out of reach on a phone.
    if (widgetPos) {
      const w = Math.min(420, window.innerWidth - 32);
      const maxX = Math.max(0, window.innerWidth - w - 8);
      const maxY = Math.max(0, window.innerHeight - 80);
      hostEl.style.left = Math.min(Math.max(0, widgetPos.x), maxX) + 'px';
      hostEl.style.top = Math.min(Math.max(0, widgetPos.y), maxY) + 'px';
  ...
  // ====== Init ======
  function init() {
    chrome.runtime.sendMessage({ type: 'GET_CAPABILITIES' }, (caps) => {
      if (!chrome.runtime.lastError && caps && typeof caps.authSupported === 'boolean') {
        authSupported = caps.authSupported;
      }
      initState();
    });
  }
